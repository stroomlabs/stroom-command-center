...
6. After shipping and pushing, smoke-test the canonical URL by curl-ing https://raw.githubusercontent.com/stroomlabs/stroom-command-center/main/apps/native/STATE.md?cachebust=$(date +%s) and verifying the new dated line appears in the output. **Always append `?cachebust=$(date +%s)` to every raw GitHub URL fetched during the smoke test** — each run hits a unique URL so GitHub's edge cache cannot serve stale bytes. Without the cachebust, raw.githubusercontent.com can return a pre-push snapshot for up to ~30 seconds after a successful push, which will cause a false "push failed" alarm. If the new content does not appear in the cachebusted response within 30 seconds, the push silently failed — surface to the operator.
