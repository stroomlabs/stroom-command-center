...
// Vertical bucket mapping. The operator's Pulse toggle has 6 buckets, each
...
//
// Source of truth for the Pulse toggle. Touching this file changes the
// pulse filter + any other surface that reads VERTICAL_BUCKETS — do not
// duplicate the mapping anywhere.
...
export const VERTICAL_BUCKETS: Record<VerticalKey, VerticalBucket> = {
  ...
  sports: {
    key: 'sports',
    label: 'Sports',
    domains: ['nfl', 'ncaa', 'nba', 'mlb', 'nhl', 'soccer', 'stadiums'],
...
// Ordered list for rendering the toggle pills. `all` first, then the
// five grouped verticals in the order the user specified.
...
// it belongs to. Returns null if the domain doesn't match any bucket.
